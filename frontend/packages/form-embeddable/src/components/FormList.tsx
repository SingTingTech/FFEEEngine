import { Button, Card, ConfigProvider, Input, Popconfirm, Space, Table, Tooltip } from 'antd';
...
export function FormList(props: FormListProps) {
  const {
    formId, apiBase, token, pageSize = 20,
  ...
  const autoColumns = useMemo(() => {
    ...
    return fields.slice(0, 5).map((f) => ({
      ...
      dataIndex: f.code,
      ellipsis: true,
      // FormRecord.data is a nested object; flatten it for display
      render: (value: any, record: FormRecord) => {
        const data = (record.data as Record<string, any>) ?? {};
        const v = data[f.code] ?? value;
